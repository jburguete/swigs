...
\title
{
	...
	{\large	English version 0.1.0 for Windows}
}
\author
{
	{\bf Author:} Javier Burguete Tolosa\\
	{\small Copyright~\copyright~2005,~20014 Javier Burguete.
	All right reserved}
}
\date{\today}
...
The programme {\swigs} is an interactive and easy-to-use tool specially
designed to perform computer simulation of one-dimensional transient flows in
all kind of river and channel beds. 

There are two versions of the code for the two operative systems  Linux and
Windows. The code has been written in C language and based on the graphic
facilities offered by \emph{GTK+} and \emph{FreeGLUT} libraries, on the
internationalization offered by \emph{Gettext} and para la gestión de ficheros
XML la librería \emph{libxml2}. These are all free software libraries under LGPL
